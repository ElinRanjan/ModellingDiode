function [strV,strI,strErrV,strErrI]=cvd(matD,matR,matV,node)
matV1=matV;
matD1=matD;
matR1=matR;
ans=MNA_CVD(matD,matR,matV)
...
if isempty(matRev)
    for i=1:size(matD,1)
        ...
        tempV=sprintf('diode %d is forward and voltage is %d V\n',...
            matD(i,1),dV);
        strV=char({strV;tempV});
        
        dI=ans(node+size(matV,1)+i);
        tempI=sprintf('diode %d is forward and current is %d A\n',...
            matD(i,1),dI);
    ...
    matR=[matR; matRev]
    ansnew=MNA_CVD(matD,matR,matV1)
